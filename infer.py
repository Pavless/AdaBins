...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0'):
        ...
        if dataset == 'nyu':
            ...
            print('here')
            model = UnetAdaptiveBins.build(n_bins=128, patch_size=12, min_val=self.min_depth, max_val=self.max_depth)
            pretrained_path = "checkpoints/exp1_5_dataRescale_75.pt"
        elif dataset == 'kitti':
            self.min_depth = 1e-3
            self.max_depth = 80
    ...
    def predict_pil(self, pil_image, visualized=False):
        pil_image = pil_image.resize((640*3//4, 480*3//4))
